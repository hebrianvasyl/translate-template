	...
	{foreach $replies as $reply}
		<li class="b-comment" {if $reply@iteration > 3} hidden{/if} name="reply">
			<div class="b-comment-info b-comment-header">
				<div class="b-comment-info-i comment-author">{if $reply.name}{$reply.name}{else}Анонимный пользователь{/if}</div>
...
{if count($replies) > 3}
	<a href="#" class="all-replies-link xhr underline" name="show_all">Читать все ответы</a>
{/if}
