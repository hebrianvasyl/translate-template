...
{if $record.sell_status=="available"}
	<div class="g-l-i-content-i-wrap available">
		...
		{if !empty($record.price)}
			<table {if !empty($record.old_price) && ($record.old_price > $record.price)}class="has_old_price"{/if} style="margin: 0px;">
				<tr>
					<td>
					...
					{if !empty($record.old_price) && ($record.old_price > $record.price)}
						<td>
							<div class="old-price-b"><span class="submit-btn old-price recent-price-old-btn">{$record->old_price nofilter}</span></div>
						</td>
					{/if}
				</tr>
			</table>
